...
class BusinessPipeline:
    ...
    def execute(self, idea: dict) -> dict:
        ...
        c.store.save_proposal(p, "proposed")
        cv=c.glados.review(p)
        if not cv.approved:
            c.store.save_proposal(p, "rejected_compliance")
        ...
        if p.expected_cost_pence:
            ...
            if not tv.approved:
                c.store.save_proposal(p, "rejected_treasury")
        ...
        if not qa.approved:
            c.store.save_proposal(p, "rejected_qa")
        ...
        price=c.hk.price(p); publication=c.r2.publish(p,artifact,price); marketing=c.j5.market(p)
